...
// holding onto it across invocations. `max: 5` keeps each warm instance's own pool small, since
// Supabase's pooler (port 6543, transaction mode) is already doing the real multiplexing — this
// just avoids one function instance alone opening more upstream connections than it needs.
...
export function getDb(): Db {
  ...
  if (supabaseUrl) {
    ...
    cachedDb = drizzleNodePostgres({ client: new Pool({ connectionString: supabaseUrl, ssl: { rejectUnauthorized: false }, max: 5 }) })
  } else {
    // Local dev's Netlify DB branch is a plain local Postgres process with no SSL at all.
    cachedDb = drizzleNodePostgres({ client: new Pool({ connectionString: getConnectionString(), max: 5 }) })
  }
  return cachedDb
}
